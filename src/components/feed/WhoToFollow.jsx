...
export default function WhoToFollow() {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      // Fetch a few users (in a real app, you'd use a smarter query)
      ...
      const users = snap.docs
        ...
        .filter(u => 
            u.id !== user.uid && // Not me
            !userData.following?.includes(u.id) // Not already following
        )
  ...
  return (
    <GlassCard className="bg-slate-900/60 border-indigo-500/20 backdrop-blur-xl mt-6">
      ...
      <div className="space-y-4">
        {suggestions.map(suggestedUser => (
            <div key={suggestedUser.id} className="flex items-center justify-between gap-3 group">
