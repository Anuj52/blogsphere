    ...
    <div className="space-y-6 pb-20">
      ...
      {savedPosts.length === 0 && (
        <div className="text-center py-20">
            ...
            <p className="text-white/30">You haven't bookmarked anything yet.</p>
        </div>
      )}

      {savedPosts.map(post => (
        <GlassCard key={post.id} className="hover:bg-slate-800/50 transition">
           <span className="inline-block px-2 py-0.5 bg-indigo-500/20 text-indigo-300 text-[10px] rounded-md mb-2 uppercase tracking-wider">
              {post.category || "General"}
           </span>
           <h3 className="font-bold text-xl text-white mb-2">{post.title}</h3>
           <p className="text-indigo-100/80 line-clamp-2 mb-4">{post.content}</p>
